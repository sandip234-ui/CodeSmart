    ...
    <Card className="mb-2 ">
      <div className="flex gap-2 items-center justify-between h-20 rounded-t-lg p-4">
        <div className="flex gap-2 items-center justify-start">
          ...
          <div>
            <h3 className="font-bold text-lg">CodeSmart</h3>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56">
            ...
            >
              <Settings size={14} strokeWidth={1.5} /> Configure API Key
            </DropdownMenuItem>
            <DropdownMenuItem
...
const ContentPage: React.FC = () => {
  ...
  const openConfiguration = () => {
    // Try to open options page first
    if (chrome.runtime.openOptionsPage) {
      chrome.runtime.openOptionsPage();
    } else {
      // Fallback to opening index.html
      window.open(chrome.runtime.getURL('index.html'));
    }
  };
  ...
  const handleModelChange = async (v: ValidModel) => {
    if (v) {
      try {
        ...
        const { apiKey } = await getKeyModel(v)
        setApiKey(apiKey)
      } catch (error) {
  ...
  return (
    ...
    >
      {!modal || !apiKey ? (
        <Card className="mb-5 w-[400px]">
          <CardContent className="p-6">
            <div className="grid place-items-center gap-4">
              <Bot className="w-12 h-12 text-primary" />
              <h2 className="text-xl font-bold text-center">CodeSmart Bot Setup</h2>
              
              {!selectedModel ? (
                <>
                  <p className="text-center text-muted-foreground">
                    Please configure the extension to start using AI assistance.
                  </p>
                  <Button
                ...
                <>
                  <div className="text-center space-y-2">
                    <p className="text-muted-foreground">
                      No API key found for <span className="font-semibold">{
                        VALID_MODELS.find(m => m.name === selectedModel)?.display
                      }</span>
                    </p>
                    <p className="text-sm text-muted-foreground">
                      You can either configure this model or select another one:
                    </p>
                  </div>
              ...
              <p className="text-xs text-center text-muted-foreground mt-4">
                Need help?{' '}
                <a
                  href="https://github.com/piyushgarg-dev/leetcode-whisper-chrome-extension#readme"
                  target="_blank"
        ...
        <ChatBox
          ...
          model={modal}
          apikey={apiKey}
          heandelModel={handleModelChange}
